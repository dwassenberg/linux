		...
		struct hid_usage *usage, unsigned long **bit, int *max)
{
	...
	// ignore usb intf 2 because it is handled by rmi
...
static int lenovo_probe_tpx1cover_special_functions(struct hid_device *hdev)
{
	...
	if (!hid_validate_values(hdev, HID_OUTPUT_REPORT, 9, 0, 2)) {
		ret = -ENODEV;
		hid_err(hdev,
			"Output report used for LED control missing\n");
		goto err;
	}
	...
	if (!report_match)
		hid_warn(hdev,
			"HID report mitmatch. Maybe it will work anyway\n");
...
static int lenovo_probe_tpx1cover(struct hid_device *hdev)
{
	...
	/*
	 * Register USB interface 0 and 1 of Lenovo X1 ThinKeyboard
	 * interface 0: general keyboard
	 * interface 1: special thinpad buttons and functions
	 * interface 2: touchpad + touchstick combo (do not register: handled by rmi!)
	 */
	...
	if (intf->cur_altsetting->desc.bInterfaceNumber == 0) {
		...
		ret = lenovo_probe_tpx1cover_special_functions(hdev);
	} else {
		// other, handled by rmi
		ret = -ENODEV;
	}
